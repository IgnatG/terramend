...
// auth.json during the run, the refreshed token must land back in Lintel
// even when the main step died unexpectedly.
//
// THIS IS WHY `CODEX_AUTH_JSON` HAS TO LIVE IN LINTEL'S OWN SECRET STORE,
// NOT IN GITHUB ACTIONS SECRETS. The refresh chain rotates on every use; this
// hook PUTs the rotated chain back to Lintel Postgres so the next run starts
...
// `lintel auth codex` next time the chain breaks.
//
...
async function main(): Promise<void> {
  ...
  try {
    ...
    core.info("codex post-hook: refreshed CODEX_AUTH_JSON persisted to Lintel");
  } catch (err) {
