...
2. **scan**: call \`${t("terraform_scan")}\` to get the best-practice \`concerns\` plus \`groups\` — one group per file, each with a stable \`id\`, its \`file\`, the group \`severity\` (highest in the file), the distinct \`rule_ids\`, and the \`concern_ids\` it covers. If it reports zero concerns, call \`${t("report_progress")}\` with "Terraform already follows best practice — nothing to remediate." and **stop**.
   ...
   *Multi-root repos (§MCP):* call \`${t("terraform_roots")}\` — when it reports more than one root (e.g. \`terraform/\` and \`terraform/core/\`), each is a separate \`init\`/\`plan\` target. \`${t("terraform_scan")}\` already covers the whole tree, but run \`${t("terraform_validate")}\` / \`${t("terraform_plan")}\` once **per root** (set \`cwd\` to each) so each root's validity + real-world effect is checked, not just the default one.
...
4. **for the chosen group**:
   ...
   - **validate**: call \`${t("terraform_validate")}\`. If it does not pass, fix what it reports or abandon this group — **never open a PR whose validate did not pass**. Its \`providers\` field carries the pinned provider majors (use them as above).
...
4. **generate (best practice from the first line)**: write the Terraform with your native file tools, guided by the \`terraform-best-practices\` skill. Requirements:
   ...
   - **Module layout + examples (§28)**: when you generate a **reusable module**, follow the standard layout (\`main.tf\` / \`variables.tf\` (typed + \`description\` + validation) / \`outputs.tf\` / \`versions.tf\` / \`README.md\`) and add at least one \`examples/basic/main.tf\` fixture that consumes it with a provider block + pinned version — this is the living documentation and the test surface. When building a \`module\` block that consumes an EXISTING module, call \`${t("terraform_module_interface")}\` on its dir first to use its real variable names. When the \`terratest\` input is enabled, also call \`${t("scaffold_terratest")}\` (module name + its dir; pass the module's variables from \`${t("terraform_module_interface")}\`) to generate a plan-only Go Terratest test **and** a Terraform-native \`tests/*.tftest.hcl\`, and write the returned files. Terramend never runs the tests (no cloud creds) — note in the PR that they should be run in the user's pipeline.
