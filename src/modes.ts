...
4. **generate (best practice from the first line)**: write the Terraform with your native file tools, guided by the \`terraform-best-practices\` skill. Requirements:
   ...
   - **Module layout + examples (§28)**: when you generate a **reusable module**, follow the standard layout (\`main.tf\` / \`variables.tf\` (typed + \`description\` + validation) / \`outputs.tf\` / \`versions.tf\` / \`README.md\`) and add at least one \`examples/basic/main.tf\` fixture that consumes it with a provider block + pinned version — this is the living documentation and the test surface. When the \`terratest\` input is enabled, also call \`${t("scaffold_terratest")}\` (module name + its dir) to generate a plan-only Go Terratest smoke test + example, and write the returned files. Terramend never runs the test (it holds no cloud creds) — note in the PR that the suite should be run in the user's pipeline.
