...
4. **generate (best practice from the first line)**: write the Terraform with your native file tools, guided by the \`terraform-best-practices\` skill. Requirements:
   ...
   - **Prefer modules**: call \`${t("list_modules")}\` first — when the operator configured a \`module_catalogue\`, build on those approved modules (registry or house, using their exact variable names, pinned). Otherwise use a well-maintained public registry module (e.g. the \`terraform-aws-modules\` collection, pinned) over hand-rolled resources when one cleanly fits. **Terratest/examples (§28):** when you generate a reusable module, also add a minimal \`examples/\` fixture (and a Terratest stub if the repo conventionally ships one) so the module is usable and testable — see the \`terraform-best-practices\` skill.
