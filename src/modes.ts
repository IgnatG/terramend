...
4. **for the chosen group**:
   ...
   - **plan (optional, safety gate)**: call \`${t("terraform_plan")}\` to attach the fix's real-world effect. It auto-skips (returns \`ran: false\`) when no cloud credentials / the terraform CLI are present, or init/plan can't complete — then add nothing. When it returns \`ran: true\`, add a one-line **Plan** note to the PR body (e.g. \`Plan: +0 ~1 -0\`). **If \`has_destroy_or_replace\` is true, call it out prominently** — list the \`destructive\` resources (address + action) and flag for careful human review, because a best-practice remediation should rarely destroy or replace a resource; if the destruction looks unintended, abandon the change rather than open a PR that would delete infrastructure.
