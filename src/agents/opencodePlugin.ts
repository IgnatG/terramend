 ...
 * `<XDG_CONFIG_HOME>/opencode/plugin/lintel-events.ts`. The harness already
 ...
 * plugin — `LINTEL_OPENCODE_GATE_PLUGIN_SOURCE` below — because it's the
 ...
 * `lintel_bus_event` — picked to be unmistakably ours so a future opencode
 * release that introduces a coincidentally-named event type won't collide.
 */

import { SUBAGENT_DENIED_TOOLS } from "./subagentToolGates.ts";

export const LINTEL_BUS_EVENT_TYPE = "lintel_bus_event" as const;

export const LINTEL_OPENCODE_PLUGIN_FILENAME = "lintel-events.ts" as const;

export const LINTEL_OPENCODE_GATE_PLUGIN_FILENAME = "lintel-subagent-gate.ts" as const;

/**
 * Source written verbatim to `<XDG_CONFIG_HOME>/opencode/plugin/lintel-events.ts`.
 *
 ...
 */
export const LINTEL_OPENCODE_PLUGIN_SOURCE = `// AUTOGENERATED by Lintel. do not edit; it'll be overwritten on the next run.
// surfaces opencode subagent activity that the CLI's run-loop discards. see
// action/agents/opencodePlugin.ts in lintel/app for why this exists. lives
...
// (lintel-subagent-gate.ts).

const LINTEL_BUS_EVENT_TYPE = ${JSON.stringify(LINTEL_BUS_EVENT_TYPE)};
...
export default async function lintelEventsPlugin() {
  return {
    event: async (input: {
      event: {
        type: string;
 ...
 * `<XDG_CONFIG_HOME>/opencode/plugin/lintel-subagent-gate.ts`. Installed by
 ...
 */
export const LINTEL_OPENCODE_GATE_PLUGIN_SOURCE = `// AUTOGENERATED by Lintel. do not edit; it'll be overwritten on the next run.
// hard-blocks state-mutating MCP tool calls from opencode subagents (PR
// following lintel/app#796 — see action/agents/subagentToolGates.ts). see
// action/agents/opencodePlugin.ts in lintel/app for why this exists. lives
// inside the per-run tmpdir (XDG_CONFIG_HOME/opencode/plugin/), never inside
...
// opencode presents MCP tools as \`lintel_<bare>\`; strip that to compare
...
// list anyway.
function stripLintelPrefix(toolName: string): string {
...
export default async function lintelSubagentGatePlugin() {
  return {
    "tool.execute.before": async (
      ...
      const bare = stripLintelPrefix(tool);
